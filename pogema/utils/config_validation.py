...
class List_Seeds(BaseModel):
    use_list_seeds: bool = False
    num_parts: int = 3
    part: int = 3
    p_use_seed: float = 0.6

    
class Stage(BaseModel, extra=Extra.forbid):
    grid_config: GridConfig = None
    termination: Termination = None
    use_seeds: List_Seeds = List_Seeds()

    @validator('termination')
    def not_empty_termination_condition(cls, v, values):
        if v is None:
            raise KeyError("Termination condition should be specified!")
        return v


class Autocurriculum(BaseModel):
...
class Environment(BaseModel, extra=Extra.forbid):
    ...
    curriculum: List[Stage] = None
    path_to_grid_configs: str = None
    autocurriculum: Autocurriculum = Autocurriculum()
    # autocurriculum: str = None
    # autocurriculum_reverse_goal: bool = False
