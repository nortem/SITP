...
from level_replay.level_sampler_curriculum import RewardCSR
...
def train(args, seeds):
    wandb.init(project='curriculum-PLR', config=args)
    ...
    if args.full_train_distribution:
        ...
        seeds = None
        level_sampler_curriculum_args = None
    elif args.autocurriculum: #CURR
        num_levels = 1
        level_sampler_args = None
        level_sampler_curriculum_args = dict(
            num_envs=args.num_processes, 
            c_name=args.curriculum_type,
            a_smooth=args.a_smooth
        )
    else:
        num_levels = 1
        level_sampler_curriculum_args = None
    ...
    envs, level_sampler = make_lr_venv(
        ...
        level_sampler_curriculum_args=level_sampler_curriculum_args)

    # reward2CSR = RewardCSR(reward_type='max', min_reward=12)
    reward2CSR = RewardCSR(reward_type=args.reward_type, min_reward=args.min_reward)
    ...
    freq_seeds = [0 for _ in range(len(seeds))]
    ...
    for j in range(num_updates):
        actor_critic.train()
        log_rewards = [0 for _ in range(args.num_processes)]
        ...
        log_seeds = [info['level_seed'] for info in infos]
        reward2CSR.reward_info(log_rewards)
        
        if args.autocurriculum: #CURR
            level_sampler.curriculum_update(log_seeds, log_rewards, reward2CSR)
        ...
        if (j % args.log_interval == 0 and len(episode_rewards) > 1) or j == num_updates - 1:
            ...
            stats = { 
                ...
                "sps": sps,
                "SR": np.mean(np.array([reward2CSR.reward_to_CSR(reward) for reward in log_rewards])),
                "p_max": reward2CSR.info.get('p.max', 0),
                "p_min": reward2CSR.info.get('p.min', 0),  
            }
            if is_minigrid:
            ...
            plogger.log(stats)
            wandb.log(stats)
            for seed_ in log_seeds:
                # wandb.log({'current-seed': seed_})
                freq_seeds[seed_] += 1
            # if args.verbose:
    ...
    # for indx_ in range(len(freq_seeds)):
    #         data = [[x, y] for (x, y) in zip(indx_, freq_seeds[indx_])]
    #         table = wandb.Table(data=data, columns = ["x", "y"])
    #         wandb.log({"freq seeds" : wandb.plot.line(table, "x", "y",
    #                 title="freq seeds")})
